...
export default function EditTreatmentPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const treatmentResponse = await fetch(`http://localhost:3001/treatments/${treatmentId}`);
        const treatmentData: Treatment = await treatmentResponse.json();
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href={`/dashboard/treatments/${treatmentId}`} className="text-blue-600 hover:text-blue-800 flex items-center">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-6 flex justify-end space-x-3">
            <Link href={`/dashboard/treatments/${treatmentId}`} className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
              Cancel
            </Link>
            <button
              type="submit"
