...
export default function EditOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  }, [order.amount, order.discountAmount, order.finalAmount]);

  const handleChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    setOrder(prev => ({
      ...prev,
      [name]: name === 'customerId' || name === 'projectId' || name === 'consultantId' || name === 'discountApproverId'
        ? parseInt(value) || 0 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/dashboard/orders/${updatedOrder.id}`);
      } else {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href={`/dashboard/orders/${orderId}`} className="text-blue-600 hover:text-blue-800 flex items-center">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h1 className="text-2xl font-bold text-gray-900">Edit Order #{order.id}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="discountApproverId" className="block text-sm font-medium text-gray-700">Discount Approver</label>
                  <select
                    id="discountApproverId"
                    name="discountApproverId"
                    value={order.discountApproverId || ''}
                    onChange={handleChange}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                  >
          ...
          <div className="mt-6 flex justify-end space-x-3">
            <Link href={`/dashboard/orders/${orderId}`} className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
              Cancel
            </Link>
            <button
              type="submit"
