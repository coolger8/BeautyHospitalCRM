  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to create campaign. Status:', response.status, 'Response:', errorText);
        ...
        alert('Failed to create campaign: ' + errorText || 'Unknown error');
      }
    } catch (error) {
    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h1 className="text-2xl font-bold text-gray-900">{t('campaigns.addNew')}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-6 flex justify-end space-x-3">
            <Link href="/dashboard/campaigns" className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
              {t('common.cancel')}
            </Link>
            <button
              type="submit"
            ...
            >
              {saving ? 'Saving...' : t('common.create') + ' ' + t('campaigns.management')}
            </button>
          </div>
        </form>
      </div>
